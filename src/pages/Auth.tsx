      ...
      <div className="w-full max-w-6xl mx-auto px-margin-mobile lg:px-margin-desktop relative z-10 flex flex-col lg:flex-row gap-10 lg:gap-16 items-center">
        ...
        <div className="flex-1 w-full space-y-6 lg:space-y-8 animate-fade-in-up">
          <div className="space-y-4">
            ...
            <p className="text-base md:text-lg text-on-surface-variant max-w-md mt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
            <div className="flex gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-on-surface">Vendeurs Vérifiés</h3>
                <p className="text-sm text-on-surface-variant mt-1">Qualité artisanale et authenticité garantie sur chaque boutique.</p>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-on-surface">Paiement Mobile</h3>
                <p className="text-sm text-on-surface-variant mt-1">Transactions 100% sécurisées via MTN, Moov, Wave en FCFA.</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative h-64 rounded-2xl overflow-hidden shadow-xl mt-8">
            ...
            <div className="absolute bottom-4 left-4 right-4 text-on-secondary">
              <p className="text-xs font-bold uppercase opacity-80 mb-1">Impact Direct</p>
              <p className="text-lg font-bold">Déjà plus de 15,000 artisans propulsés par Elevate.</p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md lg:shrink-0 bg-surface rounded-[32px] p-8 shadow-2xl relative z-10 transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="flex bg-surface-container-low rounded-xl p-1 mb-8" role="tablist">
            <button 
              ...
              className={`flex-1 py-3 text-center text-sm font-bold uppercase rounded-lg transition-all duration-200 ${isLogin ? 'bg-surface shadow-sm text-primary' : 'text-on-surface-variant hover:text-on-surface'}`}
              role="tab"
            >
              Se Connecter
            </button>
          ...
          {isLogin ? (
            <form className="space-y-5 animate-fade-in" onSubmit={(e) => e.preventDefault()}>
              <div className="space-y-2">
                <label className="text-xs font-bold text-on-surface uppercase" htmlFor="login-email">Email ou Numéro (MoMo)</label>
                <input className="w-full h-12 bg-surface-container-lowest px-4 rounded-xl text-sm text-on-surface border border-outline-variant/30 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" id="login-email" placeholder="ex: +229 97 00 00 00" type="text"/>
              </div>
              
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-on-surface uppercase" htmlFor="login-password">Mot de passe</label>
                  <a className="text-sm text-primary hover:underline" href="#">Oublié ?</a>
                </div>
                <div className="relative">
                  <input className="w-full h-12 bg-surface-container-lowest px-4 rounded-xl text-sm text-on-surface border border-outline-variant/30 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all pr-12" id="login-password" placeholder="••••••••" type={showPassword ? 'text' : 'password'}/>
            ...
            <form className="space-y-5 animate-fade-in" onSubmit={(e) => e.preventDefault()}>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-on-surface uppercase" htmlFor="reg-prenom">Prénom</label>
                  <input className="w-full h-12 bg-surface-container-lowest px-4 rounded-xl text-sm text-on-surface border border-outline-variant/30 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" id="reg-prenom" type="text"/>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-on-surface uppercase" htmlFor="reg-nom">Nom</label>
                  <input className="w-full h-12 bg-surface-container-lowest px-4 rounded-xl text-sm text-on-surface border border-outline-variant/30 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" id="reg-nom" type="text"/>
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-on-surface uppercase" htmlFor="reg-contact">Email ou Téléphone</label>
                <input className="w-full h-12 bg-surface-container-lowest px-4 rounded-xl text-sm text-on-surface border border-outline-variant/30 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" id="reg-contact" type="text"/>
              </div>
              
              <div className="space-y-2">
                <label className="text-xs font-bold text-on-surface uppercase" htmlFor="reg-password">Mot de passe</label>
                <div className="relative">
                  <input className="w-full h-12 bg-surface-container-lowest px-4 rounded-xl text-sm text-on-surface border border-outline-variant/30 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all pr-12" id="reg-password" placeholder="••••••••" type={showPassword ? 'text' : 'password'}/>
                  <button className="absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface" type="button" onClick={() => setShowPassword(!showPassword)}>
                    <span className="material-symbols-outlined text-[20px]">{showPassword ? 'visibility_off' : 'visibility'}</span>
                  </button>
                </div>
                <p className="text-xs text-on-surface-variant mt-1">8 caractères minimum</p>
              </div>
              
              <button className="w-full h-14 mt-6 bg-primary text-on-primary rounded-xl text-sm font-bold uppercase tracking-wide hover:bg-surface-tint shadow-md hover:shadow-lg transition-all transform active:scale-[0.98] flex justify-center items-center gap-2" type="submit">
                Créer mon compte
